...
def fetch_times(freq: str):
    if freq not in ['1', '5', '10', '15', '30', 'd']:
        ...
        times = ['090{}00'.format(minute) if minute < 10 else '09{}00'.format(minute) for minute in
                 range(30, 60, int(freq))]
        for hour in [str(i) for i in range(10, 16)]:
            times += ['{}0{}00'.format(hour, minute) if minute < 10 else '{}{}00'.format(hour, minute) for minute in
                      range(0, 60, int(freq))]
        times += ['160000']
        return times
...
def fetch_data_stock(stock: str, bgn_date: str, end_date: str, freq: str, cols=None, data_path=DATA_PATH):
    ...
    data = pd.DataFrame()
    for file in sorted(
            [i for i in os.listdir(data_path) if i.split('.')[-1] >= bgn_date and i.split('.')[-1] <= end_date]):
        ...
        if freq != '1':
            ...
            for idx, time in enumerate(times[1:]):
                ...
                df_freq.loc[idx, 'vwap'] = np.nansum(tmpdf.vwap.values * tmpdf.trade_volume.values) / np.nansum(
                    tmpdf.trade_volume.values)
